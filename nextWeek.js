...
setTimeButton.addEventListener( 'click', function () {
  ...
  startOrEndTimeButton.style.backgroundColor = '#1F51FF';
...
function getStartTimes(){ 
   ...
   for (let i = 0; i < startButtonTimes.length; i++) {
       ...
       startButtonTimes[i].style.backgroundColor = '#1F51FF'
...
function getEndTimes(){   
   ...
   for (let i = 0; i < endButtonTimes.length; i++) {
       ...
       endButtonTimes[i].style.backgroundColor = '#1F51FF'
...
function editTodoItem(e) {
    ...
    let todos = JSON.parse(localStorage.getItem('todosNextWeek'))
    ...
    todos.forEach(function(element, i) {
        if (inputValue === element) {
            Array.from(inputElements).forEach(element => element.addEventListener('blur', function(){
                ...
                localStorage.setItem('todosNextWeek', JSON.stringify(todos))              
...
function saveLocalTodos(todo) {
    ...
    if (localStorage.getItem('todosNextWeek') === null) {
        ...
        todos = JSON.parse(localStorage.getItem('todosNextWeek'))
    }
    todos.push(todo);
